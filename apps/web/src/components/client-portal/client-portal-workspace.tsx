...
export function ClientPortalWorkspace() {
  ...
  const organisationName =
    dashboard?.accessGrants[0]?.organisationName ?? "BusinessOS";
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-950">
        ...
        <div className="mx-auto flex max-w-6xl items-center justify-between gap-4 px-4 py-4 sm:px-6">
          <div className="flex min-w-0 items-center gap-3">
            ...
            <div className="min-w-0">
              <p className="font-semibold">Secure client portal</p>
          ...
          <form action={portalLogout}>
            <button className="inline-flex h-10 items-center gap-2 rounded-xl border border-slate-300 px-4 text-sm font-semibold">
      ...
      <main className="mx-auto max-w-6xl space-y-6 px-4 py-8 sm:px-6">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
          <div>
            ...
            <p className="mt-2 text-sm text-slate-600">
              View authorised progress, exchange messages and upload requested
              evidence.
            </p>
          </div>
          <button
        ...
        {!loading && dashboard && dashboard.accessGrants.length === 0 ? (
          <section className="rounded-2xl border border-amber-200 bg-amber-50 p-6">
            <h2 className="font-semibold text-amber-950">
              No active client access
            </h2>
            <p className="mt-2 text-sm leading-6 text-amber-900">
              Use the invitation link sent by your case team, and sign in with
              the exact verified email address to activate access.
            </p>
          </section>
        ) : null}
          ...
          <>
            <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
              <div className="rounded-2xl border border-slate-200 bg-white p-5">
                ...
                <p className="text-sm text-slate-600">authorised matters</p>
              </div>
              <div className="rounded-2xl border border-slate-200 bg-white p-5">
                <Bell className="h-5 w-5 text-blue-700" />
                <p className="mt-3 text-2xl font-bold">
                  {
                    dashboard.notifications.filter((item) => !item.readAt)
                      .length
                  }
                </p>
                <p className="text-sm text-slate-600">unread notices</p>
              </div>
              <div className="rounded-2xl border border-slate-200 bg-white p-5">
                ...
                <p className="text-sm text-slate-600">secure conversations</p>
              </div>
              <div className="rounded-2xl border border-slate-200 bg-white p-5">
                ...
                <p className="text-sm text-slate-600">billing documents</p>
              </div>
            </section>
            ...
            >
              {(["progress", "messages", "documents", "billing"] as const).map(
                (item) => (
                  <button
                    key={item}
                    type="button"
                    ...
                    className={
                      view === item
                        ...
                        : "rounded-xl border border-slate-300 bg-white px-4 py-2.5 text-sm font-semibold capitalize"
                    }
                  >
                    {item}
                  </button>
                    ...
                    >
                      <div className="flex flex-wrap justify-between gap-3">
                        <div>
                          ...
                          <h3 className="mt-1 font-semibold">{matter.title}</h3>
                        </div>
                        <span className="h-fit rounded-full bg-blue-50 px-3 py-1 text-xs font-semibold text-blue-800">
                      ...
                      <div className="mt-4 space-y-3">
                        {dashboard.updates
                          .filter((update) => update.matterId === matter.id)
                          .map((update) => (
                            <div
                              key={update.id}
                  ...
                  {dashboard.notifications.length === 0 ? (
                    ...
                    dashboard.notifications.map((notification) => (
                      <article
                        ...
                        className={
                          notification.readAt
                            ? "rounded-xl border border-slate-200 bg-white p-4 opacity-70"
                    ...
                    <button
                      ...
                      className={
                        conversationId === conversation.id
                          ...
                          : "w-full rounded-xl border border-slate-200 p-4 text-left"
                      }
                    >
            ...
            {view === "documents" ? (
              <div className="space-y-6">
                <section className="rounded-2xl border border-slate-200 bg-white p-5">
                  <h2 className="text-xl font-semibold">Requested documents</h2>
                  <div className="mt-4 space-y-4">
                    {dashboard.documentRequests.length === 0 ? (
                      <p className="text-sm text-slate-600">
                        No active document requests.
                      </p>
                    ) : (
                      dashboard.documentRequests.map((request) => (
                        <article
                          key={request.id}
                ...
                <section className="rounded-2xl border border-slate-200 bg-white p-5">
                  ...
                  <div className="mt-4 grid gap-3 sm:grid-cols-2">
                    {dashboard.documents.length === 0 ? (
                      <p className="text-sm text-slate-600">
                        No clean client-visible documents are available.
                      </p>
                    ) : (
            ...
            {view === "billing" ? (
              <section className="space-y-4">
                <h2 className="text-xl font-semibold">Invoices and payments</h2>
                {dashboard.invoices.length === 0 ? (
                  <p className="rounded-2xl border border-dashed border-slate-300 p-6 text-sm text-slate-600">
                    No billing documents have been shared for your authorised
                    matters.
                  </p>
                ) : (
                  dashboard.invoices.map((invoice) => (
                    ...
                    >
                      <div className="flex flex-wrap justify-between gap-4">
                        <div>
                          <p className="text-xs font-semibold uppercase tracking-wide text-slate-500">
                            {invoice.documentNumber} · {invoice.matterNumber}
                          </p>
                          <h3 className="mt-1 font-semibold">
                            {invoice.reference ||
                              statusLabel(invoice.documentType)}
                          </h3>
                          <p className="mt-2 text-sm text-slate-600">
