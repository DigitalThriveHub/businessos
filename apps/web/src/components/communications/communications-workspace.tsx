...
export function CommunicationsWorkspace({
  ...
  const inviteMatters = useMemo(
    () =>
      matters.filter(
        (matter) =>
          !invite.clientId || matter.primaryClientId === invite.clientId,
      ),
      ...
      await mutate(
        {
          ...
          payload: {
            ...
            recipientAddresses:
              compose.channel === "EMAIL" ? [compose.recipient] : [],
      ...
      await mutate(
        {
          ...
          payload: {
            ...
            recipientAddress:
              conversation.channel === "PORTAL" ? null : reminder.recipient,
    ...
    <div className="space-y-6">
      <header className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-600">
            Secure portal conversations, controlled email delivery, client
                ...
                <button
                  ...
                  className={
                    selectedConversationId === conversation.id
                      ? "w-full rounded-xl border border-slate-950 bg-slate-50 p-4 text-left"
                      : "w-full rounded-xl border border-slate-200 p-4 text-left hover:bg-slate-50"
                  }
                >
              ...
              >
                ...
                <div className="grid gap-4 sm:grid-cols-2">
                  ...
                  <label className="text-sm font-medium">
                    Matter
                    <select
                      required={compose.channel === "PORTAL"}
                      value={compose.matterId}
                      onChange={(event) =>
                        setCompose({ ...compose, matterId: event.target.value })
                      }
                      className="mt-2 h-11 w-full rounded-xl border border-slate-300 px-3"
                    >
                      <option value="">
                        {compose.channel === "PORTAL"
                          ? "Select matter"
                          : "Client-level conversation"}
                      </option>
                      {matters
                        .filter(
                          (matter) =>
                            !compose.clientId ||
                            matter.primaryClientId === compose.clientId,
                        )
                        .map((matter) => (
                  ...
                  <label className="text-sm font-medium">
                    ...
                    <select
                      ...
                      onChange={(event) =>
                        setCompose({ ...compose, channel: event.target.value })
                      }
                      className="mt-2 h-11 w-full rounded-xl border border-slate-300 px-3"
                    >
                ...
                <label className="block text-sm font-medium">
                  ...
                  <textarea
                    ...
                    value={compose.body}
                    onChange={(event) =>
                      setCompose({ ...compose, body: event.target.value })
                    }
              ...
              <section className="rounded-2xl border border-slate-200 bg-white p-5">
                ...
                {canSend ? (
                  <form onSubmit={submitReply} className="mt-4 flex gap-3">
                    <label className="sr-only" htmlFor="communication-reply">
                      Reply
                    </label>
                    <textarea
      ...
      {!loading && dashboard && view === "portal" ? (
        <div className="grid gap-6 xl:grid-cols-2">
            ...
            >
              ...
              <label className="block text-sm font-medium">
                ...
                <select
                  ...
                  onChange={(event) =>
                    setInvite({ ...invite, matterId: event.target.value })
                  }
                  className="mt-2 h-11 w-full rounded-xl border border-slate-300 px-3"
                >
                  <option value="">Select authorised matter</option>
                  {inviteMatters.map((matter) => (
                    <option key={matter.id} value={matter.id}>
                      {matter.matterNumber} · {matter.title}
                    </option>
                  ))}
              ...
              <p className="text-xs leading-5 text-slate-500">
                The account must sign in with this exact verified email. Access
                is limited to the selected matter.
              </p>
              <button
                disabled={busy !== null}
                className="inline-flex h-11 items-center gap-2 rounded-xl bg-slate-950 px-4 text-sm font-semibold text-white disabled:opacity-50"
              >
                <ShieldCheck className="h-4 w-4" /> Send secure invitation
              </button>
            </form>
          ) : null}
          ...
          {canPublishUpdates ? (
            ...
            >
              <div className="flex items-center gap-2">
                ...
                <h2 className="font-semibold">Publish case progress</h2>
              </div>
              <label className="block text-sm font-medium">
                Matter
                <select
                  ...
                  onChange={(event) =>
                    setProgress({ ...progress, matterId: event.target.value })
                  }
                  className="mt-2 h-11 w-full rounded-xl border border-slate-300 px-3"
                >
              ...
              <label className="block text-sm font-medium">
                ...
                <input
                  ...
                  onChange={(event) =>
                    setProgress({ ...progress, title: event.target.value })
                  }
                  className="mt-2 h-11 w-full rounded-xl border border-slate-300 px-3"
                />
              </label>
              <label className="block text-sm font-medium">
                ...
                <textarea
                  ...
                  onChange={(event) =>
                    setProgress({ ...progress, summary: event.target.value })
                  }
                  className="mt-2 w-full rounded-xl border border-slate-300 p-3"
                />
              </label>
              <div className="grid gap-4 sm:grid-cols-2">
                <label className="text-sm font-medium">
                  Stage key
                  <input
                    value={progress.stageKey}
                    onChange={(event) =>
                      setProgress({ ...progress, stageKey: event.target.value })
                    }
                    className="mt-2 h-11 w-full rounded-xl border border-slate-300 px-3"
                  />
                </label>
              ...
              <button
                disabled={busy !== null}
                className="h-11 rounded-xl bg-slate-950 px-4 text-sm font-semibold text-white disabled:opacity-50"
              >
                Publish to portal
              </button>
            </form>
          ...
          <section className="rounded-2xl border border-slate-200 bg-white p-5">
            ...
            <div className="mt-4 space-y-3">
              {dashboard.invitations.length === 0 ? (
                <p className="text-sm text-slate-600">No invitations.</p>
              ) : (
                dashboard.invitations.map((item) => (
                  ...
                  >
                    ...
                    {canManagePortal && item.status === "PENDING" ? (
                      <button
                        type="button"
                        onClick={() =>
                          void reasonAction("invitation.revoke", item.id)
                        }
          ...
          <section className="rounded-2xl border border-slate-200 bg-white p-5">
            <h2 className="font-semibold">Active portal access</h2>
            <div className="mt-4 space-y-3">
              {dashboard.accessGrants.length === 0 ? (
                <p className="text-sm text-slate-600">No access grants.</p>
              ) : (
                dashboard.accessGrants.map((item) => (
                  <article
                    key={item.id}
                    className="rounded-xl border border-slate-200 p-4"
                  >
                    ...
                    <p className="mt-2 text-xs text-slate-500">
                      {item.matters
                        .map((matter) => matter.matterNumber)
                        .join(", ") || "No matter grants"}
                    </p>
                    {canManagePortal && item.status === "ACTIVE" ? (
                      <button
                        type="button"
                        onClick={() =>
                          void reasonAction("access.revoke", item.id)
                        }
                        className="mt-3 text-sm font-semibold text-red-700"
                      >
                        Revoke access
        ...
        <div className="grid gap-6 xl:grid-cols-2">
          {canManageTemplates ? (
            <form
              ...
              className="space-y-4 rounded-2xl border border-slate-200 bg-white p-5"
            >
              <h2 className="font-semibold">Create an approved template</h2>
              <div className="grid gap-4 sm:grid-cols-2">
                <label className="text-sm font-medium">
                  Key
                  <input
              ...
              <label className="block text-sm font-medium">
                ...
                <select
                  ...
                  onChange={(event) =>
                    setTemplate({ ...template, channel: event.target.value })
                  }
                  className="mt-2 h-11 w-full rounded-xl border border-slate-300 px-3"
                >
                  <option value="EMAIL">Email</option>
              ...
              <label className="block text-sm font-medium">
                ...
                <textarea
                  ...
                  value={template.body}
                  onChange={(event) =>
                    setTemplate({ ...template, body: event.target.value })
                  }
                  className="mt-2 w-full rounded-xl border border-slate-300 p-3"
                />
              </label>
          ...
          {canManageReminders ? (
            ...
            >
              <div className="flex items-center gap-2">
                ...
                <h2 className="font-semibold">Schedule a reminder</h2>
              </div>
              <label className="block text-sm font-medium">
                Conversation
                <select
              ...
              <label className="block text-sm font-medium">
                ...
                <input
                  ...
                  onChange={(event) =>
                    setReminder({ ...reminder, subject: event.target.value })
                  }
                  className="mt-2 h-11 w-full rounded-xl border border-slate-300 px-3"
                />
              </label>
              <label className="block text-sm font-medium">
                ...
                <textarea
                  ...
                  value={reminder.body}
                  onChange={(event) =>
                    setReminder({ ...reminder, body: event.target.value })
                  }
                  className="mt-2 w-full rounded-xl border border-slate-300 p-3"
                />
              </label>
              ...
              <button
                ...
                className="inline-flex h-11 items-center gap-2 rounded-xl bg-slate-950 px-4 text-sm font-semibold text-white disabled:opacity-50"
              >
                <Clock3 className="h-4 w-4" /> Schedule reminder
              </button>
            </form>
          ) : null}
          <div className="space-y-6">
            <section className="rounded-2xl border border-slate-200 bg-white p-5">
              <h2 className="font-semibold">Reminder delivery log</h2>
              <div className="mt-4 space-y-3">
                {dashboard.reminders.length === 0 ? (
                  <p className="text-sm text-slate-600">No reminders.</p>
                ) : (
                  dashboard.reminders.map((item) => (
                    ...
                    >
                      ...
                      <p className="mt-2 text-xs text-slate-500">
                        {formatDateTime(item.scheduledFor)} · {item.channel}
                      </p>
                      {canManageReminders && item.status === "SCHEDULED" ? (
                        <button
                          type="button"
                          onClick={() =>
                            void reasonAction("reminder.cancel", item.id)
                          }
                          className="mt-3 text-sm font-semibold text-red-700"
                        >
                          Cancel reminder
            ...
            <section className="rounded-2xl border border-slate-200 bg-white p-5">
              ...
              <div className="mt-4 space-y-3">
                {dashboard.deliveryEvents.length === 0 ? (
                  <p className="text-sm text-slate-600">
                    No delivery evidence is visible for your role.
                  </p>
                ) : (
                  dashboard.deliveryEvents.map((item) => (
                    <article
                      key={item.id}
