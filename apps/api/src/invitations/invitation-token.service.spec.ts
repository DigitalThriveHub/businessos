...
describe('InvitationTokenService', () => {
  ...
  beforeEach(async () => {
    ...
    service = module.get<InvitationTokenService>(
      InvitationTokenService,
    );
  });
  ...
  it('issues a versioned 256-bit token and its deterministic HMAC hash', () => {
    ...
    expect(issued.tokenHash).toMatch(
      /^[a-f0-9]{64}$/,
    );
    ...
    const expectedHash = createHmac(
      ...
      .update(
        `businessos:organisation-invitation:v1:${issued.token}`,
        'utf8',
      )
      .digest('hex');
  ...
  it('issues a different token for every invitation', () => {
    ...
    expect(second.tokenHash).not.toBe(
      first.tokenHash,
    );
  });
  ...
  it('rejects malformed tokens before reading the secret', () => {
    expect(() =>
      service.hashToken('invalid-token'),
  ...
  it('fails closed when the HMAC secret is unavailable', () => {
    ...
    expect(() =>
      service.hashToken(VALID_TOKEN),
    ).toThrow(ServiceUnavailableException);
  });

  it('validates only the exact supported token format', () => {
    expect(
      service.isValidFormat(VALID_TOKEN),
    ).toBe(true);
    expect(
    ...
    expect(service.isValidFormat(null)).toBe(
      false,
    );
  });
});
