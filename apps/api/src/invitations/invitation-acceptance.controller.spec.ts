...
describe('InvitationAcceptanceController', () => {
  ...
  it('passes only the authenticated user and validated token to the service', async () => {
    ...
    acceptanceServiceMock.accept.mockResolvedValue(expected);

    await expect(controller.accept(user, dto)).resolves.toBe(expected);

    expect(acceptanceServiceMock.accept).toHaveBeenCalledWith(user, RAW_TOKEN);
  });
});
