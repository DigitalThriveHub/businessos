...
jest.mock('../auth/verified-jwt-payload', () => ({
...
const DATABASE_RESULT = {
  ...
  organisation_name: 'Example Legal Services',
  organisation_slug: 'example-legal-services',
  membership_id: MEMBERSHIP_ID,
  invitation_status: 'ACCEPTED',
  role_keys: ['solicitor', 'compliance_manager'],
...
describe('InvitationAcceptanceService', () => {
  ...
  const verifyUserMock = verifyUserJwtPayload as jest.MockedFunction<
    typeof verifyUserJwtPayload
  >;

  const resolveAalMock = resolveAssuranceLevel as jest.MockedFunction<
  ...
  beforeEach(async () => {
    ...
    verifyUserMock.mockReturnValue(
      TOKEN_USER as ReturnType<typeof verifyUserJwtPayload>,
    );
    resolveAalMock.mockReturnValue('AAL2');

    transaction = {
      $queryRaw: jest.fn().mockResolvedValue([DATABASE_RESULT]),
    };
  ...
  it('accepts through a user-scoped RLS transaction and returns minimal organisation data', async () => {
    await expect(service.accept(TOKEN_USER, RAW_TOKEN)).resolves.toEqual({
      invitationId: INVITATION_ID,
      organisationId: ORGANISATION_ID,
      organisationName: 'Example Legal Services',
      organisationSlug: 'example-legal-services',
      membershipId: MEMBERSHIP_ID,
      status: 'ACCEPTED',
      roleKeys: ['compliance_manager', 'solicitor'],
      jobTitle: 'Senior Solicitor',
    });

    expect(verifyUserMock).toHaveBeenCalledWith(TOKEN_USER);
    expect(resolveAalMock).toHaveBeenCalled();
    expect(tokens.hashToken).toHaveBeenCalledWith(RAW_TOKEN);
  ...
  it('rejects an invalid or expired invitation without exposing database details', async () => {
    ...
    await expect(service.accept(TOKEN_USER, RAW_TOKEN)).rejects.toThrow(
      BadRequestException,
    );
  });
  ...
  it('does not open a database transaction when token hashing rejects the token', async () => {
    ...
    await expect(service.accept(TOKEN_USER, 'invalid-token')).rejects.toThrow(
      BadRequestException,
    );
